...
@interface TabVC ()<UITableViewDataSource, UITableViewDelegate>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        cell.textLabel.text = @"SJSwitch";
    }
    
    if (indexPath.section == 1) {
        cell.textLabel.text = @"LLSegmentBar";
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    MYLog(@"%@", indexPath);
    if (indexPath.section == 0) {
        SJSwitchVC *VC = [SJSwitchVC new];
        [self.navigationController pushViewController:VC animated:YES];
    }
    
    if (indexPath.section == 1) {
        LLSegmentBarViewController *VC = [LLSegmentBarViewController new];
        [self.navigationController pushViewController:VC animated:YES];
    }

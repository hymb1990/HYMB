...
//    [exceptionInfo writeToFile:[NSString stringWithFormat:@"%@/Documents/error.log",NSHomeDirectory()]  atomically:YES encoding:NSUTF8StringEncoding error:nil];
    ...
    [self cacheExceptionLog:exceptionInfo];
    
    
}


#pragma mark 将崩溃日志缓存到本地
- (void)cacheExceptionLog:(NSString *)exceptionInfo {
    
    NSString *cachesPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).lastObject;
    //文件夹路径
    NSString *filesPath = [cachesPath stringByAppendingPathComponent:@"CashierSDKLogFile/"];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    //判断日志文件夹路径是否存在
    BOOL existed = [fileManager fileExistsAtPath:filesPath];
    if (!existed) {
    ...
    //将日志数据缓存到本地
    NSString *filePath = [filesPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@-bugLog.txt", [Util getCurrentTime]]];
    //    NSData *fileData = [[NSData alloc]initWithBase64EncodedString:exceptionInfo options:NSDataBase64DecodingIgnoreUnknownCharacters];
    NSData *fileData =[exceptionInfo dataUsingEncoding:NSUTF8StringEncoding];
